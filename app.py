...
MAIL_USERNAME = os.environ.get("MAIL_USERNAME") or "jayesh.d.chaudhary@slrtce.in"
MAIL_PASSWORD = os.environ.get("MAIL_PASSWORD") or "dkyi xcun djwe eicd"
...
ISHARIO_DB_CONFIG = {
    "host": os.environ.get("ISHARIO_DB_HOST", "localhost"),
    "user": os.environ.get("ISHARIO_DB_USER", "root"),
    "password": os.environ.get("ISHARIO_DB_PASS", "Jayesh@45"),
    "database": os.environ.get("ISHARIO_DB_NAME", "ishario_db")
}

SIGNEASE_DB_CONFIG = {
    "host": os.environ.get("SIGNEASE_DB_HOST", "localhost"),
    "user": os.environ.get("SIGNEASE_DB_USER", "root"),
    "password": os.environ.get("SIGNEASE_DB_PASS", "Jayesh@45"),
    "database": os.environ.get("SIGNEASE_DB_NAME", "signease")
}
...
# ---------- Database Helper ----------
def get_db_connection(db_config=ISHARIO_DB_CONFIG):
    return mysql.connector.connect(**db_config)

# ---------- Profile Table Ensure ----------
def ensure_profiles_table_exists():
    conn = get_db_connection(ISHARIO_DB_CONFIG)
    cursor = conn.cursor()
    create_sql = """
...
def fetch_sign_images():
    db = get_db_connection(SIGNEASE_DB_CONFIG)
    cursor = db.cursor()
...
try:
    ...
    print("[OK] Model loaded successfully from model.h5")
except Exception as e:
    ...
    try:
        ...
        print("[OK] Model loaded successfully with h5py verification")
    except Exception as e2:
        print(f"[ERROR] Alternative method failed: {e2}")
        print("\nSuggestions to fix this error:")
        print("1. Regenerate the model using train_model.py")
        print("2. Convert model.h5 to SavedModel format (.pb)")
        print("3. Check if model.h5 is corrupted and needs to be retrained")
        model = None
...
try:
    ...
    print("[WARN] Hand hand tracking will be skipped, but prediction endpoint may still work")
    hands = None
...
def dashboard():
    ...
    email = session['email']
    conn = get_db_connection(SIGNEASE_DB_CONFIG)
...
def serve_sign(filename):
    return send_from_directory(SIGN_FOLDER, filename)
