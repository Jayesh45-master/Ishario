...
def check_model_file():
    ...
    if not model_path.exists():
        print("[X] model.h5 NOT FOUND")
        return False
    
    file_size = model_path.stat().st_size
    print("[OK] model.h5 found")
    ...
    try:
        ...
        with h5py.File("model.h5", "r") as f:
            print("[OK] Valid HDF5 file detected")
            print(f"  Contents: {list(f.keys())}")
            return True
    except Exception as e:
        print(f"[X] Invalid or corrupted HDF5 file: {e}")
        return False
...
def main():
    ...
    print("\n" + "=" * 60)
    if is_valid:
        print("[OK] Your model file is valid and should work!")
        print("Try running: python app.py")
    else:
        print("[X] Your model file is corrupted or invalid.")
